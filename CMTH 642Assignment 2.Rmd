...
```{r}
cor(USDA_Clean[,c("Calories", "Protein", "TotalFat", "Carbohydrate", "Sodium", "Cholesterol")], method = c("pearson"))
```
...
```{r}
(44440-440)/440*100
# calories has also 10000%, because calories and sodium have linear relationship
```
...
```{r}
# Since the p-value 0.8123 is greater than 0.05, we can say that the medians of two articles are not different. It has insufficient evidence to indicate any improvement about how the students retell an article
```
...
treatments <- rep(c('A','B'), each=8)
blocks <- as.factor(rep(c(1:8), 2))
friedman.test(companies~treatments|blocks)
```
...
# Since the p-value 0.03389 is less than 0.05, we conclude that there is a statistically significant difference between the medians of Company A and Company B. Packaging process is different based on weight measurements 
```
